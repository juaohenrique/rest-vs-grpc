...
@GrpcService
public class ConsultaGrpcPessoaImpl extends ConsultaPessoaGrpc.ConsultaPessoaImplBase {

    private static final Logger log = LoggerFactory.getLogger(ConsultaGrpcPessoaImpl.class);
    ...
    public void findByCpf(PessoaRequest req, StreamObserver<PessoaResponse> responseObserver) {
    	...
    	log.info("Consulta pessoa por cpf: " + req.getCpf());
    	
    	Pessoa pessoa = service.findByCpf(req.getCpf());
    	
    	PessoaResponse response = PessoaResponse.newBuilder()
			.setId(pessoa.getId())
			.setNome(pessoa.getNome())
			.setNascimento(pessoa.getNascimento())
			.setFone(pessoa.getFone())
